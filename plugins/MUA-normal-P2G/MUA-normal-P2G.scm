...
; Marvel Ultimate Alliance normal map conversion
...
(define (script-fu-mua-normalp2g Image Drawables)
    (script-fu-use-v3)

    (let* (
        (tmp (car (plug-in-decompose 1 Image Drawables "rgba" 1)))
        (Black (gimp-layer-new tmp "black" (gimp-image-get-width Image) (gimp-image-get-height Image) 2 100)) ; red had least info, blue is white
...
(script-fu-register-filter "script-fu-mua-normalp2g"
    "MUA Normal Map Conversion (Pink to Green)"
    "Convert a AGRB normal map texture (pinkish) to to DXT5nm (greenish) for Marvel Ultimate Alliance 2006. Adds it on a new Layer."
    "ak2yny"
    "ak2yny"
    "April 2025"
    "RGBA"
...
(script-fu-menu-register "script-fu-mua-normalp2g" "<Image>/Script-Fu")